...
{
    {
        ...
        for (int i = 0; i < 4; i ++)
        {
            ...
            if (i <= 1)
            {
                static_cast<Cat*>(array[i])->setIdea(0, "I am a cat");
                std::cout << static_cast<Cat*>(array[i])->getIdea(0) << std::endl;
            }
        }
        for (int i = 0; i < 4; i++)
            delete array[i];
    }
    {
        ...
        original.setIdea(-1, "Error");
        copy.setIdea(100, "Error");
        std::cout << "Original's idea[-1]: " << original.getIdea(-1);
